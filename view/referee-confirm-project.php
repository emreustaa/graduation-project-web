...
if (isset($_POST['data'])) {
    ...
    if (!$data) {
        ...
        $transectionData = ['newIdStatus' => '9', 'currentRole' => "TTO Yetkilisi", 'nextRole' => 'Girişimci'];
        ...
        if ($updateTransection) {
            ...
            if ($updateTransition) {
                ...
                $actionSonuc = $actionQuery->execute([
                    "Onaylandı ve TTO yetkilisine iletildi.",
                    $data,
                    $selectedApply['Users_idUser'],
                    1,
                ]);
                if ($actionSonuc) {
                    $sonuc['deger'] = "Proje onayı tamamlandı ve TTO Yetkilisine gönderildi.";
                } else {
