...
if (isset($_POST['data'])) {
    ...
    if (!$data) {
        ...
        $transectionData = ['newIdStatus' => '7', 'currentRole' => "Girişimci", 'nextRole' => 'Girişimci'];
        ...
        if ($updateTransection) {
            ...
            if ($updateTransition) {
                ...
                $actionSonuc = $actionQuery->execute([
                    "Proje reddilme bilgisi gönderildi.",
                    $data,
                    $selectedApply['Users_idUser'],
                    2,
                ]);
                if ($actionSonuc) {
                    $sonuc['deger'] = "Proje girişimciye başarı ile gönderildi.";
                } else {
