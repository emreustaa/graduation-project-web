...
if (isset($_GET['userId']) && isset($_GET['projectCode'])) {
    ...
    if (isset($_POST['save'])) {
        ...
        if (!$projectName) {
            ...
            if ($updateResult) {
                $sorgu2 = $db->prepare('UPDATE transitions SET fromSend=?, toSend=?, startDate=?, deadline=?, Applies_Users_idUser=? WHERE Applies_idApply=' .
                    $userApplies['idApply']);
                ...
                if ($transitionSonuc) {
                    $statuTransection = $db->prepare('UPDATE statutransections SET currentRole=?, nextRole=?,Status_idStatus=? WHERE Applies_idApply=' .
                        $userApplies['idApply']);
                    ...
                    if ($transectionSonuc) {
                        ...
                        $actionSonuc = $actionQuery->execute([
                            "Proje kaydı güncellendi ve TTO yetkilisine gönderildi",
                            $projectCode,
                            $userId,
                            0,
                        ]);
                        if ($actionSonuc) {
                            echo "<script>alert('Proje bilgileri güncellenerek TTO Yetkilisine başarılı bir şekilde iletildi.');document.location='table.php?userId=" . $_GET['userId'] . "'</script>";
                        } else {
