...
if (isset($_POST['data'])) {
    ...
    if (!$data) {
        ...
        $transectionData = ['newIdStatus' => '3', 'currentRole' => "Girişimci", 'nextRole' => 'TTO Yetkilisi'];
        ...
        if ($updateTransection) {
            ...
            if ($updateTransition) {
                ...
                $actionSonuc = $actionQuery->execute([
                    "Düzenleme talebi oluşturuldu",
                    $data,
                    $selectedApply['Users_idUser'],
                    2,
                ]);
                if ($actionSonuc) {
                    $sonuc['deger'] = "Proje düzenleme talebi girişimciye iletildi.";
                } else {
